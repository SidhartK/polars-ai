...
2. `examples/02_budgets_cache.py`: request budgets, token budgets, disk cache replay.
3. `examples/03_hydration_real_dataset.py`: partial run and `hydrate(...)`.
4. `examples/04_grouped_and_multimodal.py`: grouped text input and image input options.
...
- Provider examples require API keys; the notebooks use `FakeModel` by default.
